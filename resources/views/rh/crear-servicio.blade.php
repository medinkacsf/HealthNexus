...
<head>
    ...
    <title>Nuevo Servicio - RRHH</title>
    <style>
...
<body>
    ...
    <div class="main">
        ...
        <div class="content">
            ...
            <div class="card">
                <div class="card-header"><h3> Datos del Nuevo Servicio</h3></div>
                <div class="card-body">
                    <form method="POST" action="/rh/servicios/guardar">
                        @csrf
                        <div class="form-row">
                            <div class="form-group">
                                <label>Código *</label>
                                <input type="text" name="codigo" value="{{ old('codigo') }}" placeholder="Ej: LAB-002, IMG-003" required>
                            </div>
                            <div class="form-group">
                                <label>Departamento *</label>
                                <input type="text" name="departamento" value="{{ old('departamento') }}" placeholder="Ej: Laboratorio, Imagenología" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <label>Nombre del Servicio *</label>
                            <input type="text" name="nombre" value="{{ old('nombre') }}" placeholder="Ej: Resonancia Magnética" required>
                        </div>
                        <div class="form-group">
                            <label>Descripción</label>
                            <textarea name="descripcion" placeholder="Descripción breve del servicio...">{{ old('descripcion') }}</textarea>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label>Costo Operativo *</label>
                                <input type="number" step="0.01" min="0" name="costo_operativo" value="{{ old('costo_operativo', 0) }}" required>
                                <div class="hint">Lo que le cuesta al hospital realizar el servicio</div>
                            </div>
                            <div class="form-group">
                                <label>Precio Sugerido *</label>
                                <input type="number" step="0.01" min="0" name="precio_sugerido" value="{{ old('precio_sugerido', 0) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <select name="cuenta_id">
                                ...
                                <option value="{{ $id }}" {{ old('cuenta_id')==$id?'selected':'' }}>{{ $nombre }}</option>
                                @endforeach
                            </select>
                            <div class="hint">Cuenta financiera donde se registran los cobros</div>
                        </div>
                        <div class="form-actions">
                            <a href="/rh/servicios" class="btn btn-outline">Cancelar</a>
                            <button type="submit" class="btn btn-green"> Guardar Servicio</button>
                        </div>
                    </form>
                </div>
            </div>
